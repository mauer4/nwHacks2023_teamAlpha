    ...
    def updateGUI(self, d):
        ...
        for i in range(0, len(set)):
            if set[i] in self.history.keys():
                ...
                self.canvas.coords(a.line,a.x1,a.y1,a.x2+200,a.y2)

            else:

                distance = i*50
                line = self.canvas.create_line(x1 + self.counter * 200, y1 + distance + 10, x2 + self.counter * 200, y1 + distance + 10, arrow=LAST)
                l = myLine(x1 + self.counter * 200,y1 + distance + 10, x2 + self.counter * 200, y1 + distance + 10, set[i], line)
